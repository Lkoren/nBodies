require "C:/Liav/webdev/artComputationalScience/vector.rb"
class Body
...
class Nbody
	...
	def rk2(dt)
		...
		half_vel = []
		@body.each_index{|i| half_vel[i] = @body[i].vel + @body[i].acc(@body)*0.5*dt}
	...
	def write_diagnostics(nsteps)
		...
		print "==============" + "\n"
		print "Diagnostics:" + "\n"
		print "at time t = " + sprintf("%g", time) + ", after #{nsteps} steps : " + "\n"
		print "E_kin = " + sprintf("%.3g", ekin) + "\n"
		print "E_pot = " + sprintf("%.3g", epot) + "\n"
		print "E_tot = " + sprintf("%.3g", etot) + "\n"
		print "E_tot - E_init = " + sprintf("%.3g", etot - @e0) + "\n"
		print "(E_tot - E_init) / E_init = " + sprintf("%.3g", (etot - @e0)/@e0 ) + "\n"
		print "==============" + "\n"
		#END
	end
	...
	def simple_print
		print "total number of bodies: ", @body.size, "\n"
